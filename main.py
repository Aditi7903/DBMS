...
def TopItem():
    try:
        query = "SELECT ItemName, Quantity, CanteenID, SUM(UnitsSold) AS TotalUnitsSold \
                    FROM MENU_ITEMS \
...
def insert_canteen():
    try:
        ...
        row["OpeningYear"] = input("Opening Year (YYYY-MM-DD): ")
        row["Location"] = input("Location: ")

        query = "INSERT INTO CANTEEN (CanteenID, OpeningYear, Location) VALUES (%s, %s, %s)"
        values = (row["CanteenID"], row["OpeningYear"], row["Location"])
...
def update_menu_item_price():
    try:
        ...
        query = "UPDATE MENU_ITEMS SET Price = %s WHERE ItemName = %s AND CanteenID = %s AND Quantity = %s"
    ...
    while 1:
            try:
                ...
                if chSelect == 1:
                    ...
                    query = "select ItemName as Item, Quantity, Price, UnitsSold as 'Units Sold', CanteenID as Canteen from MENU_ITEMS where Price = {} and 1=1 ".format(price)
                    ...
                    query = "select CanteenID as Canteen, OpeningYear as 'Opening Year', Location, {} from CANTEEN where '{}' < {}".format(day, day, time)
    ...
    while 1:
        try:
            ...
            if chProj == 1:
                ...
                query = "select ItemName, Price as Item from MENU_ITEMS where Price > {} ".format(amount)
...
def SearchItem():
    ...
    try:
        prefix = input("Enter prefix: ")
        query = "SELECT ItemName as Name FROM MENU_ITEMS WHERE ItemName LIKE '{}%';".format(prefix)
        mycur.execute(query)
        output = mycur.fetchall()
        if mycur.rowcount != 0:
            print()
